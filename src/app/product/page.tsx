  ...
  parseInt(searchParams.maxPrice)
    async function fetchData() {
      ...
      if (searchParams.ordByPrice) {
        query += ` ORDER BY CAST(Price AS DECIMAL) ${
          searchParams.ordByPrice === "max" ? "DESC" : "ASC"
        }`;
      }
    ...
    <main className="flex flex-wrap flex-col content-center items-start mx-5">
      <div className="w-full inline-flex justify-between">
        {searchParams.category ? (
          <div className="gap-2 flex flex-row items-center border-gray-400 border bg-transparent rounded-md">
        ...
        {searchParams.category ? <FilterbyPriceRange /> : <div></div>}
        <OrderButtons />
      </div>
